/*
 * grunt-bizweb
 * https://github.com/HuyHL/grunt-bizweb
 *
 * Copyright (c) 2016 Huy Ha
 * Licensed under the MIT license.
 */
'use strict';

module.exports = function(grunt) {
	var bizweb = bizweb || {},
        fs = require('fs'),
        path = require('path'),
        glob = require('glob'),
        util = require('util'),
        https = require('https'),
        growl = require('growl');

    /*
     * Return the base api host with the basic auth. Does not require the
     * protocol.
     *
     * @return {string}
     */
     bizweb.getHost = function() {
        var config = grunt.config('bizweb');

        return config.options.url;
    };

    /*
     * Return the authentication header for basic auth
     *
     * @return {string}
     */
     bizweb.getAuth = function() {
        var config = grunt.config('bizweb');

        return config.options.api_key + ":" + config.options.password;
    };

    bizweb.getThemeId = function() {
        var config = grunt.config('bizweb');

        return config.options.ThemeId;
    };

    /*
     * Helper to detect whether a file is binary or not. Used to handle sending
     * image assets to bizweb vs other assets
     *
     * @param {string}
     * @param {Function}
     */
     bizweb.isBinaryFile = function(file, callback) {
        var ascii = true,
            i, len;

        fs.readFile(file, function(err, data) {
            if (err) {
                grunt.log.error("isBinaryFile failed on " + file + ": " + err);

                return false;
            }

            for (i = 0, len = data.length; i < len; i++) {
                if (data[i] > 127) {
                    ascii = false;

                    break;
                }
            }

            callback(ascii, data);
        });
    };

    /**
     * Helper for reporting Http response success and error messages to the
     * user. To notify the user without a response (i.e for an info note) simply
     * don't pass a response notify("hello");
     *
     * @param {response}|{string}
     * @param {string}
     */
     bizweb.notify = function(res, msg) {
        var config = grunt.config('bizweb');

        if (typeof res !== "string") {
            if (res.statusCode >= 400) {
                msg = "Error " + msg + " (Status Code: " + res.statusCode + ")";

                if (!config.options.disable_growl_notifications) {
                    growl(msg, {title: 'Grunt Bizweb'});
                }

                if (!config.options.disable_grunt_log) {
                    grunt.log.error('[grunt-bizweb] - ' + msg);
                }
            }
            else {
                msg = "Success " + msg + " (Status Code: " + res.statusCode + ")";

                if (!config.options.disable_growl_notifications) {
                    growl(msg, {title: 'Grunt Bizweb'});
                }

                if (!config.options.disable_grunt_log) {
                    grunt.log.ok('[grunt-bizweb] - ' + msg);
                }
            }
        }
        else {
            if (!config.options.disable_growl_notifications) {
                growl(res, {title: 'Grunt Bizweb'});
            }

            if (!config.options.disable_grunt_log) {
                grunt.log.ok('[grunt-bizweb] - ' + res);
            }
        }
    };

    /*
     * Convert a file path on the local file system to an asset path in bizweb
     * as you may run grunt at a higher directory locally.
     *
     * The original path to a file may be something like shop/assets/site.css
     * whereas we require assets/site.css in the API. To customize the base
     * set bizweb.options.base config option.
     *
     * @param {string}
     * @return {string}
     *
     * @todo
     */
     bizweb.getAssetKey = function(path) {
        var c = grunt.config('bizweb');

        if (c.options.base) {
            return path.substring(path.indexOf(c.options.base) + c.options.base.length);
        }

        return path;
    };

    /*
     * Remove a given file path from bizweb.
     *
     * File should be the relative path on the local filesystem. See
     * getAssetKey for the conversion to remote asset location
     *
     * @param {string} file
     * @param {function} async completion callback
     */
     bizweb.remove = function(file, done) {
        bizweb.notify("Deleting " + file);
        file = file.replace("\\", "/");

        var path = bizweb.getAssetKey(file).replace("\\", "/");
        if (bizweb.getThemeId() != '') {
            var options = {
                host: bizweb.getHost(),
                auth: bizweb.getAuth(),
                path: '/admin/themes/' + bizweb.getThemeId() + '/assets.json?asset[key]=' + path,
                method: 'DELETE',
                headers: {
                    'Content-Length': 0
                }
            };
        } else {
            var options = {
                host: bizweb.getHost(),
                auth: bizweb.getAuth(),
                path: '/admin/assets.json?asset[key]=' + path,
                method: 'DELETE',
                headers: {
                    'Content-Length': 0
                }
            };
        }

        var req = https.request(options, function(res) {
            res.setEncoding('utf8');

            var body = '';

            res.on('data', function(chunk) {
                body += chunk;
            });

            res.on('end', function() {
                if (res.statusCode >= 400) {
                    bizweb.notify(res, "delete failed with response " + body);
                } else {
                    bizweb.notify(res, "deleted file " + path + " from bizweb");
                }
                bizweb.notify(res, "deleting file");
                return done(true);
            });

        });

        req.on('error', function(e) {
            bizweb.notify('Problem with DELETE request: ' + e.message);

            return done(false);
        });

        req.end();

        return true;
    };

    /*
     * Upload a given file path to Bizweb
     *
     * Assets need to be in a suitable directory.
     *      - Liquid templates => "templates/"
     *      - Liquid layouts => "layout/"
     *      - Liquid snippets => "snippets/"
     *      - Theme settings => "config/"
     *      - General assets => "assets/"
     *
     * Some requests may fail if those folders are ignored
     * @param {string} file
     * @param {function} async completion callback
     */
     bizweb.upload = function(file, done) {
        file = file.replace("\\", "/");
        bizweb.notify("Uploading " + file.replace("\\", "/"));

        bizweb.isBinaryFile(file, function(ascii, data) {
            var key = bizweb.getAssetKey(file.replace("\\", "/")),
                post = {};

            if (ascii) {
                // if the file is a binary file
                post = JSON.stringify({
                    'asset': {
                        'value': data.toString('utf8'),
                        'key': key
                    }
                });
            } else {
                post = JSON.stringify({
                    'asset': {
                        'attachment': data.toString('base64'),
                        'key': key
                    }
                });
            }

            if (bizweb.getThemeId() != '') {
                var options = {
                    host: bizweb.getHost(),
                    auth: bizweb.getAuth(),
                    path: '/admin/themes/' + bizweb.getThemeId() + '/assets.json',
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'Content-Length': Buffer.byteLength(post, 'utf8')
                    }
                };
            } else {
                var options = {
                    hostname: bizweb.getHost(),
                    auth: bizweb.getAuth(),
                    path: '/admin/assets.json',
                    method: 'PUT',
                    headers: {
                        'Content-Type': 'application/json',
                        'Content-Length': Buffer.byteLength(post, 'utf8')
                    }
                };
            }

            var req = https.request(options, function(res) {
                res.setEncoding('utf8');

                var body = '';

                res.on('data', function(chunk) {
                    body += chunk;
                });

                res.on('end', function() {
                    if (res.statusCode >= 400) {
                        bizweb.notify(res, "upload failed with response " + body);
                    } else {
                        bizweb.notify(res, "uploaded file to bizweb as " + key);
                    }
                    return done(true);
                });

            });

            req.on('error', function(e) {
                bizweb.notify('Problem with PUT request: ' + e.message);

                return done(false);
            });

            req.write(post);
            req.end();
        });

        return true;
    };

    bizweb.deploy = function(done) {
        var c = grunt.config('bizweb');
        var paths = [];
        ['assets', 'config', 'layout', 'snippets', 'templates'].forEach(function(folder) {
            paths = paths.concat(glob.sync(path.join(c.options.base || '', folder, '*.*')));
        });
        function next(i) {
            if (i < paths.length) {
                bizweb.upload(paths[i].replace("\\", "/"), function(success) {
                    if (!success) {
                        return done(false);
                    }
                    next(i + 1);
                });
            } else {
                done(true);
            }
        }
        next(0);
    };

    bizweb.getOneAsset = function(key, done) {
        if (bizweb.getThemeId() != '') {
            var options = {
                host: bizweb.getHost(),
                auth: bizweb.getAuth(),
                path: '/admin/themes/' + bizweb.getThemeId() + '/assets.json?asset[key]=' + key,
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json'
                }
            };
        } else {
            var options = {
                hostname: bizweb.getHost(),
                auth: bizweb.getAuth(),
                path: '/admin/assets.json?asset[key]=' + key,
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json'
                }
            };
        }
        var req = https.request(options, function(res) {
            res.setEncoding('utf8');

            var c = grunt.config('bizweb');
            var body = '';
            var destination = path.join(c.options.base || '', key);

            res.on('data', function(chunk) {
                body += chunk;
            });

            res.on('end', function() {
                try {
                    var obj = JSON.parse(body);
                    if (obj.asset) {
                        var value, encoding;
                        if (typeof obj.asset.value !== 'undefined') {
                            value = obj.asset.value;
                            encoding = 'utf8';
                        } else if (typeof obj.asset.attachment !== 'undefined') {
                            value = new Buffer(obj.asset.attachment, 'base64');
                            encoding = null;
                        } else {
                            grunt.log.error('Parsed object is not complete: ' + util.inspect(obj));
                            return done(false);
                        }
                        if (grunt.option('no-write')) {
                            bizweb.notify(util.format('dry run: Downloaded %s to %s', key, destination));
                            console.log(util.inspect(obj));
                            done(true);
                        } else {
                            fs.writeFile(destination, value, encoding, function(err) {
                                if (err) {
                                    grunt.log.error(util.format('Error saving asset %s to %s: %s', key, destination, err.message));
                                    done(false);
                                } else {
                                    bizweb.notify(util.format('Downloaded %s to %s', key, destination));
                                    done(true);
                                }
                            });
                        }
                    } else {
                        grunt.log.error('Parsed object is not complete: ' + util.inspect(obj));
                        return done(false);
                    }
                } catch (e) {
                    grunt.log.error('Failed to parse JSON response');
                    done(false);
                }
            });
        });

        req.on('error', function(e) {
            bizweb.notify('Problem with GET request: ' + e.message);
            return done(false);
        });

        req.end();
    };

    bizweb.download = function(done) {

        if (bizweb.getThemeId() != '') {
            var options = {
                host: bizweb.getHost(),
                auth: bizweb.getAuth(),
                path: '/admin/themes/' + bizweb.getThemeId() + '/assets.json',
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json'
                }
            };
        } else {
            var options = {
                hostname: bizweb.getHost(),
                auth: bizweb.getAuth(),
                path: '/admin/assets.json',
                method: 'GET',
                headers: {
                    'Content-Type': 'application/json'
                }
            };
        }
        var req = https.request(options, function(res) {
            res.setEncoding('utf8');

            var body = '';
            var obj;

            function next(i) {
                if (i < obj.assets.length) {
                    bizweb.getOneAsset(obj.assets[i].key, function(success) {
                        if (!success) {
                            return done(false);
                        }
                        next(i + 1);
                    });
                } else {
                    bizweb.notify('Theme sync complete.');
                    done(true);
                }
            }
            ;

            res.on('data', function(chunk) {
                body += chunk;
            });

            res.on('end', function() {
                try {
                    obj = JSON.parse(body);
                    if (obj.assets) {
                        next(0);
                    } else {
                        grunt.log.error('Failed to get bizweb assets');
                        return done(false);
                    }
                } catch (e) {
                    grunt.log.error('Failed to parse JSON response');
                    done(false);
                }
            });
        });

        req.on('error', function(e) {
            bizweb.notify('Problem with GET request: ' + e.message);
            return done(false);
        });

        req.end();
    };

    /*
     * Bizweb noop.
     *
     * Use watch to monitor changes. To do an initial upload of all files on
     * your local copy, use the bizweb upload functionality.
     */
    grunt.registerTask('bizweb', function() {
        return true;
    });

    grunt.registerTask('bizweb:download', 'Downloads a single theme file from bizweb, or the entire theme if no file is specified', function(p) {
        var done = this.async();
        if (p) {
            var key = bizweb.getAssetKey(p);
            bizweb.getOneAsset(key, done);
        } else {
            bizweb.download(done);
        }
    });

    grunt.registerTask('bizweb:upload', 'Uploads a single theme file to Bizweb, or the entire theme if no file is specified', function(p) {
        var done = this.async();
        if (p) {
            bizweb.upload(p, done);
        } else {
            bizweb.deploy(done);
        }
    });

    grunt.registerTask('bizweb:delete', 'Removes a theme file from Bizweb', function(p) {
        var done = this.async();
        bizweb.remove(p.replace("\\", "/"), done);
    });

    /**
     * Grunt watch event
     */
    grunt.event.on('watch', function(action, filepath) {
        var upload = true;

        try {
            if (fs.lstatSync(filepath).isDirectory()) {
                upload = false;
            }
        } catch (e) {
            //
        }

        if (upload) {
            switch (action) {
                case 'deleted':
                    bizweb.remove(filepath.replace("\\", "/"), function() {
                    });

                    break;
                case 'added':
                case 'changed':
                case 'renamed':
                    bizweb.upload(filepath.replace("\\", "/"), function() {
                    });

                    break;
            }
        } else {
            bizweb.notify("Skipping directory " + filepath);
        }

        return true;
    });
}